...
-- GitHub Edition для Real Executor
...
do
    ...
    function B:ToBase64()
        ...
        for i = 1, math.ceil(#self.bits / 6) * 6 do
            s = s + pow2[c] * (self.bits[i] or 0)
            c = c + 1
            if c >= 6 then
                table.insert(r, base64chars:sub(s + 1, s + 1))
                c, s = 0, 0
            end
        end
...
pcall(function()
    ...
    if mf then
        ...
        if bm then
            local BlocMap = require(bm)
            if BlocMap and BlocMap.GrandeurBloc then
                GrandeurBloc = BlocMap.GrandeurBloc
            end
        end
    end
...
local function buildHouse(bx, by, bz, w, d, h, wall, floor, roof, glass, door)
    ...
    for x = -1, w do for z = -1, d do
        placeBlock(bx + x, by - 1, bz + z, Color3.fromRGB(100, 100, 100), "Concrete")
    end end
            ...
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    if z == 0 and x == dX and y < 2 then
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(150, 100, 50), door)
                    elseif (z == 0 or z == d - 1) and y >= 1 and y <= 2 and (x == dX - 2 or x == dX + 2 or x == 1 or x == w - 2) then
                        ...
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(200, 180, 160), wall)
                    end
                end
            end
        end
...
local function buildTower(bx, by, bz, w, d, h, wall, glass)
            ...
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    if y % 2 == 0 then
                        ...
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(200, 200, 200), wall)
                    end
                end
            end
        end
    ...
    for x = -1, w do for z = -1, d do
        placeBlock(bx + x, by + h, bz + z, Color3.fromRGB(150, 100, 50), "Slate")
    end end

    for y = 1, 3 do
        placeBlock(bx + math.floor(w / 2), by + h + y, bz + math.floor(d / 2), Color3.fromRGB(255, 200, 50), "Metal")
    end
    print("✅ Вышка готова!")
end
...
local function buildCastle(bx, by, bz, w, d, h, wall, roof)
    ...
    for x = -2, w + 1 do for z = -2, d + 1 do
        placeBlock(bx + x, by - 1, bz + z, Color3.fromRGB(80, 80, 80), "Concrete")
    end end
    ...
    for y = 0, h - 1 do
        for x = 0, w - 1 do
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(180, 160, 140), wall)
                end
            end
        end
    end
    ...
    for _, t in ipairs({{0, 0}, {w - 1, 0}, {0, d - 1}, {w - 1, d - 1}}) do
        ...
        for x = -1, 1 do for z = -1, 1 do
            placeBlock(bx + tx + x, by + h + 2, bz + tz + z, Color3.fromRGB(150, 80, 50), roof)
        end end
    end
    print("✅ Замок готов!")
end
...
local function buildTemple(bx, by, bz, w, d, h, wall, roof)
    print("🏛️ Строим храм...")
    for x = -2, w + 1 do for z = -2, d + 1 do
        placeBlock(bx + x, by - 1, bz + z, Color3.fromRGB(150, 150, 150), "Marble")
    end end
    ...
    for y = 0, h - 1 do
        for x = 0, w - 1 do
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(220, 210, 190), wall)
                end
            end
        end
    end
...
local function buildLighthouse(bx, by, bz, w, d, h, wall, glass)
    ...
    for y = 0, h - 1 do
        ...
        for x = off, w - 1 - off do
            for z = off, d - 1 - off do
                if x == off or x == w - 1 - off or z == off or z == d - 1 - off then
                    if y >= h - 2 then
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(100, 200, 255), glass)
                    else
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(200, 100, 50), wall)
                    end
                end
            end
        end
    ...
    for x = -1, 1 do for z = -1, 1 do
        placeBlock(bx + math.floor(w / 2) + x, by + h, bz + math.floor(d / 2) + z, Color3.fromRGB(255, 255, 100), "Glass")
    end end
    print("✅ Маяк готов!")
end

local function buildSkyscraper(bx, by, bz, w, d, h, wall, glass)
    print("🏢 Строим небоскрёб...")
    for x = -2, w + 1 do for z = -2, d + 1 do
        placeBlock(bx + x, by - 1, bz + z, Color3.fromRGB(80, 80, 80), "Concrete")
    end end
            ...
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    if y % 2 == 0 then
                        ...
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(150, 150, 170), wall)
                    end
                end
            end
        end
    ...
    for y = 1, 4 do
        placeBlock(bx + math.floor(w / 2), by + h + y, bz + math.floor(d / 2), Color3.fromRGB(200, 50, 50), "Metal")
    end
    print("✅ Небоскрёб готов!")
end

local function buildChurch(bx, by, bz, w, d, h, wall, roof, glass)
    print("⛪ Строим церковь...")
    for x = -2, w + 1 do for z = -2, d + 1 do
        placeBlock(bx + x, by - 1, bz + z, Color3.fromRGB(100, 100, 100), "Concrete")
    end end
            ...
            for z = 0, d - 1 do
                if x == 0 or x == w - 1 or z == 0 or z == d - 1 then
                    if y == 1 and (x == math.floor(w / 2) or z == math.floor(d / 2)) then
                        ...
                        placeBlock(bx + x, by + y, bz + z, Color3.fromRGB(220, 210, 200), wall)
                    end
                end
            end
        end
...
local function updatePreview(rot, baseGrid, baseY)
    if not builder.previewInitialized then
        ...
        builder.previewParts = {}
        for i = 1, c do
            table.insert(builder.previewParts, createPreview(Color3.new(1, 1, 1), Vector3.new(0, -100, 0)))
        end
        ...
        for z = -2, builder.depth + 1 do
            for y = -1, builder.height + 5 do
                ...
                if p then
                    ...
                    elseif x == 0 or x == builder.width - 1 or z == 0 or z == builder.depth - 1 then
                        p.Color = Color3.fromRGB(200, 180, 160)
...
title.Text = "🏗️ СТРОИТЕЛЬ ПОСТРОЕК v19.3"
